    ...
    <div class="container-fluid">
        <div class="tab-content mt-3">
            <div class="tab-pane fade show active " id="data-grid" role="tabpanel">
                <form class="card" id="validate-form" action="{{ route('maintenance-type.store') }}" method="post" enctype="multipart/form-data" id="submit_user_form">
                @csrf    
                <div class="card-header">
                        @lang('maintenanceType.add_new_mntns_type')
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-12"> 
                                <div class="mb-3">
                                    <div class="row">
                                        <div class="col-md-3">
                                            ...
                                            <select class="form-select form-control is-invalid" name="mntns_card_type" id="mntns_card_type" required>
                                            <option value="" selected> choose</option>    
                                            @foreach($card_list as $card)
                                                <option value="{{$card->system_code}}" > {{$card->system_code_name_ar}} - {{$card->system_code_name_en}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-3">
                                            <label for="recipient-name" class="col-form-label "> @lang('maintenanceType.mntns_type') </label>
                                            <select class="form-select form-control is-invalid" name="mntns_type" id="mntns_type" required>
                                            <option value="" selected> choose</option>    
                                            @foreach($type_cat as $mt)
                                                <option value="{{$mt->system_code}}" > {{$mt->system_code_name_ar}} - {{$mt->system_code_name_en}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-3">
                                            <label for="recipient-name" class="col-form-label">  @lang('maintenanceType.mntns_type_name_ar')   </label>
                                            <input type="text" class="form-control is-invalid" name="mntns_type_name_ar"
                                                       id="mntns_type_name_ar" placeholder="@lang('maintenanceType.mntns_type_name_ar')"
                                                       v-model="mntns_type_name_ar"
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <input type="text" class="form-control is-invalid" name="mntns_type_name_en"
                                                       id="mntns_type_name_en" placeholder="@lang('maintenanceType.mntns_type_name_en')"
                                                       v-model="mntns_type_name_en"
