            ...
            <div class="section-body mt-3" id="app">
                <div class="container-fluid">
                    ...
                    <div class="row mb-12">
                        <div class="col-md-3">
                            ...
                            @if(auth()->user()->user_type_id != 1)
                                @foreach(session('job')->permissions as $job_permission)
                                    @if($job_permission->app_menu_id == 73 && $job_permission->permission_add)
                                        <a href="{{route('Returned-invoices.create')}}" class="btn btn-primary">
                                            <i class="fe fe-plus mr-2"></i>@lang('invoice.add_invoice_return')
                                        </a>
                                    @endif
                                @endforeach
                            @else
                        ...
                        <div class="col-md-2">
                            <form action="{{route('invoices-exportreturn') }}">
                                ...
                                <button href="{{ route('invoices-exportreturn') }}" type="submit"
                                ...
                                @foreach($invoices as $k=>$invoice)
                                    <tr>
                                     ...
                                     $invoice->customer->customer_name_full_en }}</td>
                                        <td>{{ date_format($invoice->invoice_due_date,'y-m-d') }}</td>
                                        <td>{{number_format($invoice->invoice_amount,2 )}}</td>
                                        <td>{{number_format($invoice->invoice_total_payment * -1 ,2 )}}</td>
                                        <td>{{number_format($invoice->invoice_amount - $invoice->invoice_total_payment * -1 ,2 )}}</td>

                                        <td>{{ $invoice->invoice_is_payment }}</td>
                                        <td><a href="{{ route('Returned-invoices.show',$invoice->invoice_id) }}"
                                            ...
                                            <a
                                            href="{{config('app.telerik_server')}}?rpt={{$invoice->report_url_return->report_url}}&id={{$invoice->invoice_id}}&lang=ar&skinName=bootstrap"
