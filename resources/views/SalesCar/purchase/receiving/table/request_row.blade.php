@foreach($sales_request->details as $key => $d)
    ...
    @if($d->store_vou_qnt_t_o - $d->store_vou_qnt_t_i_r > 0)
        @for($i=0; $i < ($d->store_vou_qnt_t_o - $d->store_vou_qnt_t_i_r) ; $i++)
        <tr id="{{ $i }}">
            <td class="ctd">
                @if($d->store_vou_qnt_t_o - $d->store_vou_qnt_t_i_r > 0)
            ...
            <td class="ctd"> 
                ...
                <input type="hidden" class="form-control" name="{{$i}}" id="car_uuid_{{$i}}" value="0" >
            ...
            <td class="ctd">
                <input type="text" class="form-control" name="{{$i}}" id="sales_cars_add_amount_{{$i}}" value="{{ $car_data['sales_cars_add_amount'] }}"  onchange="calculateTransTotal(this)" readonly>
            </td>
            <td class="ctd"> 
                <input type="text" class="form-control total_sum" name="{{$i}}" id="store_vou_item_total_price_{{$i}}" value="{{ number_format($d->store_vou_item_total_price/$all_qty +   $car_data['sales_cars_add_amount']) }}"  readonly>  
            </td>
...
@endforeach
<input type="hidden" class="form-control" name="trans_req_type" id="trans_req_type" value="request" >