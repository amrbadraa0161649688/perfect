    ...
    <table class="table table-bordered card_table" id="item_table">
        <tbody > 
            <tr>
                ...
                <th class="ctd table-active">P No</th>
            ...
            @foreach($sales_request->details as $key => $d)
                ...
                <tr id="{{ $d->uuid }}">
                    <td class="ctd">
                        @if(optional($d->car)->sales_car_status != \App\Models\SystemCode::where('system_code','120004')->first()->system_code_id)
                        @if($d->store_vou_qnt_i - $d->store_vou_qnt_t_i_r > 0)
                        <div class="form-group">
                            <label class="custom-control custom-checkbox custom-control-inline">
                    ...
                    <!-- <td class="ctd"> 
                        {{ (number_format($d->store_vou_qnt_i) - number_format($d->store_vou_qnt_t_i_r) )}} 
                    </td> -->
                    <td class="ctd"> 
                        <input type="text" class="form-control item-qty" name="{{$d->uuid}}" id="store_vou_qnt_i_r_{{$d->uuid}}" value="{{ (number_format($d->store_vou_qnt_i) - number_format($d->store_vou_qnt_t_i_r) )}}" onchange="calculateItem(this)" readonly>
                        <input type="hidden" class="form-control" name="{{$d->uuid}}" id="store_vou_item_id_{{$d->uuid}}" value="{{ $d->store_vou_item_id}}" readonly>
                        <input type="hidden" class="form-control" name="{{$d->uuid}}" id="store_vou_qnt_i_{{$d->uuid}}" value="{{ (number_format($d->store_vou_qnt_i) - number_format($d->store_vou_qnt_t_i_r) )}}" readonly>
                    </td>
                    <td class="ctd"> 
                        <input type="text" class="form-control" name="store_vou_qnt_t_i_r" id="store_vou_qnt_t_i_r_{{$d->uuid}}" value="{{ (number_format($d->store_vou_qnt_i) - number_format($d->store_vou_qnt_t_i_r) )}}" readonly>
                    </td>
                    <td class="ctd"> 
