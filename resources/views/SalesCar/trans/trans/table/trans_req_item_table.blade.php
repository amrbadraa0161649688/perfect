    ...
    <table class="table table-bordered card_table" id="item_table">
        <tbody >
            <tr>
                ...
                <th class="ctd table-active" style="width:7%">الكمية المحولة</th>
            ...
            @foreach($sales_request->details as $key => $d)
                ...
                <tr id="{{ $d->uuid }}">
                    <td class="ctd">
                        @if($d->store_vou_qnt_r - $d->store_vou_qnt_t_i_r > 0)
                        <div class="form-group">
                            <label class="custom-control custom-checkbox custom-control-inline">
                    ...
                    <td class="ctd"> {{ (number_format($d->store_vou_qnt_r) - number_format($d->store_vou_qnt_t_i_r) )}} </td>
                    <td class="ctd"> 
                        <input type="text" class="form-control item-qty" name="{{$d->uuid}}" id="store_vou_qnt_t_o_{{$d->uuid}}" value="{{ (number_format($d->store_vou_qnt_r) - number_format($d->store_vou_qnt_t_i_r) )}}" onchange="calculateItem(this)" readonly>
                        <input type="hidden" class="form-control" name="{{$d->uuid}}" id="store_vou_qnt_r_{{$d->uuid}}" value="{{ (number_format($d->store_vou_qnt_r) - number_format($d->store_vou_qnt_t_i_r) )}}" readonly>

                        <input type="hidden" class="form-control" name="{{$d->uuid}}" id="store_brand_id_{{$d->uuid}}" value="{{ $d->store_brand_id }}" readonly>
                        <input type="hidden" class="form-control" name="{{$d->uuid}}" id="store_brand_dt_id_{{$d->uuid}}" value="{{ $d->store_brand_dt_id }}" readonly>
                    </td>
                    <td class="ctd"> 
                        <input type="text" class="form-control" name="store_vou_qnt_t_i_r" id="store_vou_qnt_t_i_r_{{$d->uuid}}" value="{{ (number_format($d->store_vou_qnt_r) - number_format($d->store_vou_qnt_t_i_r) )}}" readonly>
                    </td>
                    <td class="ctd"> 
        ...
        <tfoot>
            <tr>
               ...
               <td colspan="2" class="ctd table-active"> @lang('sales_car.total_vat_amount') </td>
                ...
                <td colspan="2" class="ctd table-active"> <div id="total_sum_net_div"> 0 </div> </td>
            </tr>
        </tfoot>
    </table>
</div>
