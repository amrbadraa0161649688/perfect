@if(auth()->user()->user_type_id != 1)
    @foreach(session('job')->permissions as $job_permission)
        @if($job_permission->app_menu_id == 65 && $job_permission->permission_add)
            <div class="row">
                <div class="col-md-4">
                    <a class="btn btn-icon" href="{{ route('store-sales-inv.edit',$row->uuid ) }}"
                       title="">
                        <i class="fa fa-eye"></i>
                    </a>
                </div>
                <div class="col-md-4">
                    @if($row->status)
                        @if($row->status->system_code != 125001)
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control date" name="bond_date"
                                       required>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                $row->company_id)->where('system_code',580003)->first()->system_code_name_ar}}">
                                @else
                                    <input type="text" class="form-control" readonly
                                           value="{{App\Models\SystemCode::where('company_id',
                                $row->company_id)->where('system_code',580003)->first()->system_code_name_en}}">
                                @endif

                                <input type="hidden" name="bond_doc_type"
                                       value="{{App\Models\SystemCode::where('company_id',
                                $row->company_id)->where('system_code',580003)->first()->system_code_id}}">
                            </div>
                        </div>
