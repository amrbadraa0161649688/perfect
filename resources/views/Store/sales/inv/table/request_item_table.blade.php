...
<div class="table-responsive" style="height:300px;">
    <input type="hidden" class="form-control" name="vat_rate" id="vat_rate" value="$('#store_acc_no :selected').data('customervatrate')" readonly>
    <table class="table table-bordered card_table" id="item_table">
        <tbody >
            <tr>
                ...
                <th class="ctd table-active" style="width:7%">كمية  المبيعات </th>
            ...
            <?php 
                ...
                $item_row_count = 0;
            ?>
            <input type="hidden" class="form-control" name="store_vou_ref_before" id="store_vou_ref_before" value="{{ $purchase_request->uuid }}" >
            @foreach($purchase_request->details as $key => $d)
                <?php 
                    ...
                    $qty = number_format($d->store_vou_qnt_q) - number_format($d->store_vou_qnt_t_i_r);
                   
                    $store_voue_disc_value = $d->store_voue_disc_value * 100;
                    $store_vou_disc_amount = $d->store_vou_disc_amount;
                    $store_vou_vat_rate = $d->purchase->customer->customer_vat_rate;
                    $store_vou_item_total_price = ($qty * ($d->store_vou_item_price_unit - ($d->store_vou_item_price_unit * $d->store_voue_disc_value) )) ;
                    $store_vou_vat_amount = ( ($store_vou_item_total_price ) * $store_vou_vat_rate );  //trans('vat.current_vat_value');
                    $store_vou_price_net =  ($store_vou_item_total_price + $store_vou_vat_amount );
                ?>
                <tr id="{{ $d->uuid }}">
                    <td class="ctd">
                        @if($d->store_vou_qnt_q - $d->store_vou_qnt_t_i_r > 0)
                        <div class="form-group">
                            <label class="custom-control custom-checkbox custom-control-inline">
                    ...
                    <td class="ctd"> {{ (number_format($d->store_vou_qnt_q) - number_format($d->store_vou_qnt_t_i_r) )}} </td>
                    <td class="ctd"> 
                        <input type="text" class="form-control item-qty" name="{{$d->uuid}}" id="store_vou_qnt_o_{{$d->uuid}}" value="{{ (number_format($d->store_vou_qnt_q) - number_format($d->store_vou_qnt_t_i_r) )}}" onchange="calculateItem(this)" readonly>
                        <input type="hidden" class="form-control" name="{{$d->uuid}}" id="store_vou_qnt_q_{{$d->uuid}}" value="{{ (number_format($d->store_vou_qnt_q) - number_format($d->store_vou_qnt_t_i_r) )}}" readonly>
                    </td>
                    <td class="ctd"> 
                        <input type="text" class="form-control" name="store_vou_qnt_t_i_r" id="store_vou_qnt_t_i_r_{{$d->uuid}}" value="{{ (number_format($d->store_vou_qnt_q) - number_format($d->store_vou_qnt_t_i_r) )}}" readonly>
                    </td>
                    <td class="ctd"> 
                    ...
                    <td class="ctd">
                        <input type="text" class="form-control" name="{{$d->uuid}}" id="store_voue_disc_value_{{$d->uuid}}" value="{{ $store_voue_disc_value }}" onchange="calculateTotal(this)"  readonly>
                    </td>
                    <td class="ctd">
                        <input type="text" class="form-control total_disc" name="{{$d->uuid}}" id="store_vou_disc_amount_{{$d->uuid}}" value="{{ $store_vou_disc_amount }}" readonly>
                    </td>
        ...
        <tfoot>
            <tr>
                ...
                <td colspan="2" class="ctd table-active"> <div id="total_sum_div"> 0</div> </td>
               ...
               <td colspan="1" class="ctd table-active"> @lang('purchase.total_vat_amount') </td>
                <td colspan="2" class="ctd table-active"> <div id="total_sum_vat_div"> 0 </div> </td>
