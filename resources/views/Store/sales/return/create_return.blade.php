<div id="create_return_modal" class="modal fade " tabindex="-1" role="dialog" aria-hidden="true" >
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content" style="width:250%">
            <div class="modal-header"> 
               
                <h4 class="modal-title" style="text-align:right">
                    @lang('sales.add_new_return') 
                </h4>
                                ...
                                <div class="form-group">
                                    <div class="custom-controls-stacked">
                                        <!-- <label class="custom-control custom-radio custom-control-inline">
                                            ...
                                            <span class="custom-control-label"> اذن استلام جديد </span>
                                        </label> -->
                                        <label class="custom-control custom-radio custom-control-inline">
                                            <input type="radio" class="custom-control-input" name="create_tyep" id="create_tyep" value="from_receiving">
                                            <span class="custom-control-label"> @lang('sales.inv_return') </span>
                                        </label>
                                    </div>
                                </div>
                            </div>

                            <div class="row" id="from_receiving" style="display:none;">
                                <div class="col-md-6">
                                    <label for="recipient-name" class="col-form-label ">@lang('sales.inv_no') </label>
                                    <input type="text" class="form-control" name="request_code" id="request_code" value="" onchange="getDataByCode(this)">
                                </div>
                                <div class="col-12" id="showResult"></div>
                            </div>
...
<script type="text/javascript">
    ...
    function calculateItem(e)
    {
        ...
        store_vou_qnt_o_r = parseFloat($('#store_vou_qnt_o_r_'+e.name).val());
        store_vou_qnt_t_i_r = parseFloat($('#store_vou_qnt_t_i_r_'+e.name).val());
        
        if(store_vou_qnt_o_r <= 0)
        {
            $('#store_vou_qnt_o_r_'+e.name).val(store_vou_qnt_o);
            $('#store_vou_qnt_t_i_r_'+e.name).val(0 );
            return toastr.warning('لايمكن اضافة كمية اقل من صفر ');
        }

        if(store_vou_qnt_o_r>store_vou_qnt_o)
        {
            $('#store_vou_qnt_o_r_'+e.name).val(store_vou_qnt_o);
            toastr.warning('لايمكن اضافة كمية اكبر من كمية الطلب');
        }
        else{   

            $('#store_vou_qnt_t_i_r_'+e.name).val(store_vou_qnt_o - store_vou_qnt_o_r );
    ...
    function calculateTotal(e)
    {
        ...
        store_vou_qnt_o_r = parseFloat($('#store_vou_qnt_o_r_'+e.name).val());
        store_vou_item_total_price_amount = unit_price * parseFloat(store_vou_qnt_o_r);
        ...
        if(store_vou_disc_type == 533)
        {
            ...
            if( store_voue_disc_value  > 100)
            {
                ...
                return  alert('لايمكن تطبيق خصم اكبر من 100%');
            }
        ...
        else
        {
            ...
            {
                $('#store_voue_disc_value_'+e.name).val(0)
                return  alert(' لا يمكن تطبيق خصم اكبر من القيمة');
            }
    ...
    function saveRequest()
    {
        ...
        $('.item-qty').each(function(){
            ...
            if(checked_item == true){
                ...
                row ={
                    'uuid' :  uuid  ,
                    'store_vou_qnt_o_r' : parseFloat($('#store_vou_qnt_o_r_'+ uuid).val()),
        ...
        url = '{{ route('store-sales-return.store') }}'
