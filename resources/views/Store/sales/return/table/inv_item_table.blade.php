...
<div class="table-responsive" style="height:300px;">
    <table class="table table-bordered card_table" id="item_table">
        ...
        $item_row_count = 0;
        ?>
        @if(count($purchase_request->details) > 0)
            @foreach($purchase_request->details as $key => $d)
                ...
                $qty = floatval($d->store_vou_qnt_o - $d->store_vou_qnt_t_i_r);
                            ...
                            <div class="form-group">
                                <label class="custom-control custom-checkbox custom-control-inline">
                                    <input type="checkbox" class="custom-control-input" id="selected_item_{{$d->uuid}}"
                                           name="{{$d->uuid}}" value="{{$d->uuid}}" onchange="selectItem(this)">
                                    <span class="custom-control-label"></span>
                                </label>
                            </div>
                        @endif
                ...
                <!-- <td class="ctd"> {{ $key + 1 }} </td> -->
                    ...
                    <td class="ctd"> {{ (($d->store_vou_qnt_o) - ($d->store_vou_qnt_t_i_r) )}} </td>
                    <td class="ctd">
                        <input type="text" class="form-control item-qty" name="{{$d->uuid}}"
                               id="store_vou_qnt_o_r_{{$d->uuid}}"
                               value="{{ (($d->store_vou_qnt_o) - ($d->store_vou_qnt_t_i_r) )}}"
                               onchange="calculateItem(this)" readonly>
                        <input type="hidden" class="form-control" name="{{$d->uuid}}" id="store_vou_qnt_o_{{$d->uuid}}"
                               value="{{ (($d->store_vou_qnt_o) - ($d->store_vou_qnt_t_i_r) )}}" readonly>
                    </td>
                    <td class="ctd">
                    ...
                    <td class="ctd">
                        <select class="form-select form-control" name="{{$d->uuid}}"
                                id="store_vou_disc_type_{{$d->uuid}}" data-live-search="true"
                                onchange="calculateTotal(this)" disabled>
                            @foreach($item_disc_type as $dt)
                                <option value="{{ $dt->system_code_id }}"
                                        data-mntnsdisctype="{{ $dt->getSysCodeName() }}"> {{ $dt->getSysCodeName() }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td class="ctd">
                        <input type="text" class="form-control" name="{{$d->uuid}}"
                               id="store_voue_disc_value_{{$d->uuid}}" value="0" onchange="calculateTotal(this)"
                               readonly>
                    </td>
                    <td class="ctd">
