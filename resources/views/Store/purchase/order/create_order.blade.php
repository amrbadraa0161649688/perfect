<div id="create_order_modal" class="modal fade full" tabindex="-1" role="dialog" aria-hidden="true" >
    <div class="modal-dialog">
        <div class="modal-content" style="width:250%">
            <div class="modal-header">
               
                <h4 class="modal-title" style="text-align:right">
                    @lang('purchase.add_new_order') 
                </h4>
                        ...
                        <div class="mb-3">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <div class="custom-controls-stacked">
                                        ...
                                        <label class="custom-control custom-radio custom-control-inline">
                                            <input type="radio" class="custom-control-input" name="create_tyep" id="create_tyep" value="from_request">
                            ...
                            <div class="row" id="from_request" style="display:none;">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" name="request_code" id="request_code" value="" onchange="getDataByCode(this)">
                                </div>
                            ...
                            <div class="row" id="new" style="display:none;">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select form-control is-invalid" name="store_vou_pay_type" id="store_vou_pay_type" required>
                                        ...
                                        <option value="{{$p_method->system_code_id}}"> {{ $p_method->getSysCodeName() }} </option>
                                        @endforeach
                                    </select>
                                </div>
            ...
            <div class="modal-footer">
                ...
                <div class="col-sm-6 col-xs-6 col-md-6 col-lg-6" id="new_btn" style="display:none;">	
                    <button type="button" id="btnSave" class="btn btn-success forward btn-block" onclick="saveDirectOrder()"> اضافة جديد </button>
                </div>
                
                <div class="col-sm-6 col-xs-6 col-md-6 col-lg-6" id="from_request_btn" style="display:none;">	
...
<script type="text/javascript">
    ...
    function calculateItem(e)
    {
        ...
        if(store_vou_qnt_p <= 0)
        {
            $('#store_vou_qnt_p_'+e.name).val(store_vou_qnt_r);
            $('#store_vou_qnt_t_i_r_'+e.name).val(0 );
            return toastr.warning('لايمكن اضافة كيمة اقل من صفر ');
        }

        if(store_vou_qnt_p>store_vou_qnt_r)
        {
            $('#store_vou_qnt_p_'+e.name).val(store_vou_qnt_r);
            ...
            $('#store_vou_qnt_t_i_r_'+e.name).val(store_vou_qnt_r - store_vou_qnt_p );
    ...
    function calculateTotal(e)
    {
        ...
        store_vou_qnt_p = parseFloat($('#store_vou_qnt_p_'+e.name).val());
        store_vou_item_total_price_amount = unit_price * parseFloat(store_vou_qnt_p);
        ...
        else
        {
            ...
            $discount_amount = getNum(store_voue_disc_value);
            $(store_vou_disc_amount).val($discount_amount);
        }
        vat_rate = 15/100;
    ...
    function calculateTotalOfAll()
    {
        ...
        $('#total_sum_div').text(getNum(total_sum));
        ...
        $('#total_disc_div').text(getNum(total_disc));
        ...
        $('#total_sum_vat_div').text(getNum(total_sum_vat));
        ...
        $('#total_sum_net_div').text(getNum(total_sum_net));
    ...
    function saveRequest()
    {
        ...
        $('#btnSave').prop('hidden', true)
        url = '{{ route('store-purchase-order.store') }}'
        var form = new FormData($('#order_form')[0]);
    ...
    function saveDirectOrder()
    {
        ...
        $('#btnSave').prop('hidden', true)
        url = '{{ route('store-purchase-direct-order.store') }}'
        var form = new FormData($('#order_form')[0]);
        form.append('company_id', $('#company_id').val());
        form.append('warehouses_type', $('#warehouses_type').val());
        form.append('branch_id', $('#branch_id').val());
        ...
        $.ajax({
            ...
            {
                toastr.success(data.msg);
                url =  '{{ route("store-purchase-direct-order.edit", ":id") }}';
                url = url.replace(':id',data.uuid);
                window.location.href = url;
