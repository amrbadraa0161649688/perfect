@if(auth()->user()->user_type_id != 1)
    @foreach(session('job')->permissions as $job_permission)
        @if($job_permission->app_menu_id == 64 && $job_permission->permission_add)
            <div class="row">
                <div class="col-md-4">
                    <a class="btn btn-icon" href="{{ route('store-purchase-receiving.edit',$row->uuid ) }}"
                       title="">
                        <i class="fa fa-eye"></i>
                    </a>
                </div>
                            ...
                            <div class="form-group">
                                ...
                                @if(app()->getLocale()=='ar')
                                    <input type="text" class="form-control" readonly value="{{\App\Models\SystemCode::where('system_code',590001)
                                                            ->where('company_group_id',$row->company_group_id)->first() ? \App\Models\SystemCode::where('system_code',590001)
