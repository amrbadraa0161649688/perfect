...
<div class="table-responsive" style="height:300px;">
    <table class="table table-bordered card_table" id="item_table">
        ...
        <tr>
            ...
            <th class="ctd table-active" style="width:7%">كمية المستودع</th>
        ...
        @foreach($purchase_request->details as $key => $d)
            ...
            <tr id="{{ $d->uuid }}">
                        ...
                        <div class="form-group">
                            <label class="custom-control custom-checkbox custom-control-inline">
                                <input type="checkbox" class="custom-control-input" id="selected_item_{{$d->uuid}}"
                                       name="{{$d->uuid}}" value="{{$d->uuid}}" onchange="selectItem(this,false)">
                                <span class="custom-control-label"></span>
                            </label>
                        </div>
                    @endif
                ...
                <td class="ctd"> {{$d->item->item_balance}}   </td>
                <td class="ctd"> {{ $d->store_vou_qnt_t_o - $d->store_vou_qnt_t_i_r }} </td>
                <td class="ctd">
                    <input type="text" class="form-control item-qty" name="{{$d->uuid}}"
                           id="store_vou_qnt_i_{{$d->uuid}}"
                           value="{{ $d->store_vou_qnt_t_o - $d->store_vou_qnt_t_i_r}}"
                ...
                <td class="ctd">
                    <input type="text" class="form-control total_sum" name="{{$d->uuid}}"
                           id="store_vou_item_total_price_{{$d->uuid}}" value="{{ $d->store_vou_item_total_price }}"
                           readonly>
                </td>

                <td class="ctd">
                    <select class="form-select form-control" name="{{$d->uuid}}" id="store_vou_disc_type_{{$d->uuid}}"
                            data-live-search="true" onchange="calculateTotal(this)" disabled>
                ...
                <td class="ctd">
                    <input type="text" class="form-control total_sum_vat" name="{{$d->uuid}}"
                           id="store_vou_vat_amount_{{$d->uuid}}" value="{{ $d->store_vou_vat_amount }}" readonly>
                </td>
                <td class="ctd">
                    <input type="text" class="form-control total_sum_net" name="{{$d->uuid}}"
                           id="store_vou_price_net_{{$d->uuid}}" value="{{ $d->store_vou_price_net }}" readonly>
                </td>
            </tr>
