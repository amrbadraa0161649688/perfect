...
class BondDiscountController extends Controller
{
...
//       انواع الخصومات
        $system_code_types = SystemCode::where('sys_category_id', 61)
            ->where('company_group_id', $company->company_group_id)->get();
        ...
        if (request()->contract_id) {
            ...
            return view('Bonds.Discounts.create', compact('applications', 'company', 'branch',
    ...
    public function store(Request $request)
    {
        ...
        Bond::create([
            ...
            'bond_type_id' => 4, ///سند خصم
            'bond_type_name' => 'Discount',
        ...
        if (isset($contract)) {
            if ($request->bond_doc_type == 5544) {   /// accident discount decrease from accident total
                $contract->contract_total_add = $contract->contract_total_add - $request->bond_amount_debit;

                $contract->contract_vat_amout = $contract->contract_vat_amout + $request->bond_vat_amount;
                $contract->contract_net_amount = $contract->contract_net_amount - $request->bond_amount_debit + $request->bond_vat_amount;
            } else {
                $contract->contract_total_discount = $contract->contract_total_discount + $request->bond_amount_debit;
    ...
    {
        $bond = Bond::find($id);
        return view('Bonds.Discount.show', compact('bond'));
    }
}
