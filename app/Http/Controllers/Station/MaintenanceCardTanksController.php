...
class MaintenanceCardTanksController extends Controller
{
...
//        الكروت المغلقه
        ...
        return view('Stations.MaintenanceCardTanks.index', compact('maintenance_cards', 'branches',
    ...
    public function create()
    {
        ...
        $selected_type_id = $mntns_cards_type->where('system_code', 480012)->first()->system_code_id;
        ...
        return view('Stations.MaintenanceCardTanks.create1', compact('mntns_cards_type', 'mntns_cards_category', 'status',
            'selected_type_id'));
    }
    ...
    public function createM()
    {
        ...
        $status = SystemCode::where('system_code', 50001)->where('company_group_id', $company->company_group_id)->first();
        return view('Stations.MaintenanceCardTanks.createM', compact('mntns_cards_type', 'mntns_cards_category', 'status',
            'selected_type_id'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $last_card_serial = CompanyMenuSerial::where('branch_id', $branch->branch_id)
            ->where('app_menu_id', 154)->latest()->first();
        ...
        if (isset($last_card_serial)) {
            ...
            $string_number = 'MNTS-T-' . session('branch')['branch_id'] . '-1';
        ...
        DB::commit();
        return redirect()->route('maintenanceCardTanks.create2', $maintenance_hd->mntns_cards_id);
    ...
    public function create2($id)
    {
        ...
        $attachments = Attachment::where('transaction_id', $id)->where('app_menu_id', 154)->get();
        $notes = Note::where('transaction_id', $id)->where('app_menu_id', 154)->get();
        $attachment_types = SystemCode::where('sys_category_id', 11)
            ->where('company_group_id', $company->company_group_id)->get();


        return view('Stations.MaintenanceCardTanks.create2', compact('maintenance_hd', 'maintenance_types',
