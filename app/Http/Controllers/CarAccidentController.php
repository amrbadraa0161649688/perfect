...
////معقب الحادث
        ...
        return view('CarAccident.index', compact('car_accidents', 'accident_types', 'accident_statuses',
    ...
    public function create()
    {
        ...
        $accident_status = SystemCode::where('company_group_id', $company->company_group_id)
            ->where('sys_category_id', 130)->where('system_code', 1300001)->first();
        ...
        if (request()->contract_id) {
            ...
            return view('CarAccident.create', compact('contract', 'attachment_types', 'accident_types', 'accident_status',
                'appreciation_sides', 'insurance_companies', 'direct_accident'));
        } else {
            return view('CarAccident.create', compact('accident_types', 'accident_status',
                'appreciation_sides', 'insurance_companies', 'direct_accident'));
        }
...
////مباشر الحادث
        ...
        return view('CarAccident.edit', compact('car_accident', 'attachment_types',
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->car_accident_type_id != $car_accident->car_accident_type_id) {
            if ($car_accident->accidentType->system_code != 129002) { ////لو القيمه مضافه علي العقد في الاضافه
                if ($car_accident_type->system_code == 129002) {
                    $car_accident->contract->contract_total_add =
                        $car_accident->contract->contract_total_add - $request->car_accident_amount;

                } else {
                    $car_accident->contract->contract_total_add = $car_accident->contract->contract_total_add + 0;
                }

                $car_accident->contract->save();

            } else {///////////////لو القميه غير مضافه علي العقد
                if ($car_accident_type->system_code == 129002) {
                    $car_accident->contract->contract_total_add =
                        $car_accident->contract->contract_total_add + 0;
                } else {
                    $car_accident->contract->contract_total_add = $car_accident->contract->contract_total_add +
                        $request->car_accident_amount;
                }
        ...
        if ($request->path == 'contract') {
            return redirect()->route('car-rent.edit', $car_accident->contract->contract_id);
        } else {
            return redirect()->route('car-accident.index');
        }
